...
const featuredProjects = [
  {
    ...
    index: '01',
    year: '2025',
    title: 'Transparent RAG System',
    subtitle: 'Hybrid Retrieval-Augmented Generation with Full Observability',
    description: 'A production-grade, local RAG system featuring multi-format ingestion, hybrid retrieval fused via Reciprocal Rank Fusion, cross-encoder re-ranking, and citation confidence gates. Complete pipeline observability with Langfuse and an evaluation suite mapping retrieval and generation quality.',
    results: [
      'Multi-format Ingestion: Ingests PDFs, Markdown documents, and web pages into a persistent ChromaDB',
      'Hybrid Retrieval & Re-ranking: BM25 + semantic search fused via RRF, re-scored with ms-marco-MiniLM-L-6-v2',
      'Citation Enforcement: Restricts outputs to verified chunks with a confidence-gated threshold to prevent hallucination',
      'Observability & Evals: Full span tracing in Langfuse and custom golden Q&A dataset benchmarks (Recall, Precision, MRR, NDCG)',
    ],
    stack: ['Python', 'ChromaDB', 'Ollama', 'sentence-transformers', 'Langfuse', 'Gradio', 'LangChain', 'pypdf'],
  ...
  },
  {
    ...
    year: '2026',
    title: 'InterviewAI',
    subtitle: 'Real-time Voice AI Senior Engineering Interviewer',
    description: 'An elite, production-grade, real-time voice AI system that conducts live, low-latency technical interviews over voice, dynamically adapts its follow-up questions, challenges candidate reasoning, and generates comprehensive multi-metric performance feedback.',
    results: [
      'Sub-1.2s end-to-end latency WebSocket pipeline with Deepgram ASR, local Ollama LLM, and ElevenLabs TTS HTTP streaming',
      'Event-driven interruption engine with active playback queue flushing and immediate cancellation signal coroutines',
      'Asymmetric interim speech debouncer merging fragmented transcripts with an 800ms silence detection timeout',
      'Dedicated thread isolation pooling for evaluation generation preventing WebSockets event loop starvation',
    ],
    stack: ['Python 3.11', 'FastAPI', 'WebSockets', 'Ollama', 'Deepgram', 'ElevenLabs', 'React', 'Vite', 'SQLite'],
    link: 'https://interview-ai-ochre-gamma.vercel.app/',
    github: 'https://github.com/JKSANJAY27/InterviewAI',
    image: '/projects/interviewai.png',
    accent: '#D02020',
  },
  {
  ...
  {
    category: 'AI Engineering • NLP Pipeline',
    index: '04',
    year: '2026',
    title: 'Prefilter AI',
    subtitle: 'Natural Language → Structured Search Queries, Instantly',
    description: 'Fine-tuned Qwen3.5-0.8B LoRA adapter that converts natural language search queries into structured filters across 10 domains. Exposes a full query-understanding middleware pipeline — parser, ontology engine, conflict detector, relaxer, and multi-backend translators — via a REST API and real-time web dashboard.',
    results: [
      'Fine-tuned Qwen3.5-0.8B LoRA: Extracts only explicitly mentioned fields, never hallucinating values, with operator-prefix output (lt:, gte:, ne:, between:) ready for zero-NLP downstream filters',
      'Multi-backend translators: IR → SQL (parameterized), Elasticsearch DSL, MongoDB, and ChromaDB filter clauses in a single pipeline call',
      'Conflict detection & relaxation: Feasibility checks, contradiction detection, and targeted constraint relaxation with full explanation provenance',
      'Tri-parser support: Rule-based spaCy (~1–5 ms), local SLM, or Gemini API, selectable per-request; multi-turn stateful sessions via REST',
    ],
    stack: ['Python', 'FastAPI', 'Qwen3.5-0.8B', 'LoRA / PEFT', 'spaCy', 'Hugging Face', 'Uvicorn', 'Gemini API'],
    link: 'https://github.com/JKSANJAY27/Prefilter-AI',
    github: 'https://github.com/JKSANJAY27/Prefilter-AI',
    image: '/projects/prefilter.png',
    accent: '#1040C0',
  },
];
