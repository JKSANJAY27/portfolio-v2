...
void mainImage(out vec4 fc,in vec2 frag){
  ...
  float ax=abs(nx);float stMag=mix(ax,pow(ax,FOG_TILT_SHAPE),0.35);float st=sign(nx)*stMag*uTiltScale;
...
export const LaserFlow: React.FC<Props> = ({
    ...
    useEffect(() => {
        ...
        const { r, g: gv, b } = hexToRGB(color || '#7c3aed');
        const uniforms = {
            ...
            uFogFallSpeed: { value: fogFallSpeed }, uColor: { value: new THREE.Vector3(r, gv, b) }, uFade: { value: 0 }
        };
        ...
        const setSizeNow = () => {
            ...
            (uniforms.iResolution.value as THREE.Vector3).set(w * baseDpr, h * baseDpr, baseDpr);
        ...
        const onMove = (ev: PointerEvent) => {
            ...
            mouseTarget.set(x * baseDpr, (rect.height - y) * baseDpr);
        };
        ...
        const animate = () => {
            ...
            (uniforms.uFlowTime as { value: number }).value += cdt;
            (uniforms.uFogTime as { value: number }).value += cdt;
            if (!hasFadedRef.current) { fade = Math.min(1, fade + cdt); (uniforms.uFade as { value: number }).value = fade; if (fade >= 1) hasFadedRef.current = true; }
    ...
    useEffect(() => {
        ...
        if (!uniforms) return;
        const { r, g: gv, b } = hexToRGB(color || '#7c3aed');
        (uniforms.uColor.value as THREE.Vector3).set(r, gv, b);
